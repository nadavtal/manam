...
const projectData = props => {
  ...
  const tabs = [
    {name: 'Info', icon: 'info', tabNumber: 1},
    {name: 'Models', icon: 'cubes', tabNumber: 2},
    {name: 'Processes', icon: 'tasks', tabNumber: 3},
    {name: 'Images', icon: 'camera', tabNumber: 4},
    {name: 'Spans', icon: 'cube', tabNumber: 5},
  ]
  ...
  const toggleModal = (modalType, objectId) => {
    ...
    switch (modalType) {
      case 'allocateToSpan':
        ...
        break;
      case 'editElement':
        const item = props.bridgeElements.find(el => el.object_id == objectId)
        props.onToggleModal({
  ...
  const Tabs = () => {
    return tabs.map(tab =>  <IconName 
      onClick={() => toggleClassicTabs3(tab.tabNumber)}
      name={tab.name}
      iconName={tab.icon}
      active={activeItemClassicTabs3 === tab.tabNumber}
      className='py-1'
      />)
  }
